...
function isInsideCircle(circle, point) {
  const num1 = (circle.center.x - point.x) ** 2;
  const num2 = (circle.center.y - point.y) ** 2;
  const distance = Math.sqrt(num1 + num2);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (isStartIncluded === true) {
    res1.push(tru[0]);
  } else { res1.push(fal[0]); }
  res1.push(sort[0]);
  res2.push(sort[1]);
  if (isEndIncluded === true) {
    res2.push(tru[1]);
  } else { res2.push(fal[1]); }
  res3 = `${res1.join('')}, ${res2.join('')}`;
  return res3;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (typesOpenBrac.includes(str[i])) {
      bracStack.push(str[i]);
    } else {
      if (bracStack.length === 0) {
        return false;
      }
      if (pairForBrac[str[i]] === bracStack[bracStack.length - 1]) {
        bracStack.pop();
      } else {
        return false;
      }
